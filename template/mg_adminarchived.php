<?php
/* TODO: Create Archive
 *  create template for archived messages for group as shortcode
 *
 * */
...
if ($totcount > 0) {
    ...
    <script type="text/javascript">
        ...
        jQuery(document).ready(function () {
            ...
            jQuery('body').on('click', '.quick_view', function () {
                jQuery('#archivelist > tbody  > tr').each(function () {
                    jQuery(this).closest('tr').css("background-color", "#F9F9F9");
                });
                jQuery(this).closest('tr').css("background-color", "#FEA03D");
                var thisId = this.name;
                var data = {action: 'wpmg_viewmessage', page: 'wpmg_mailinggroup_viewmessage', id: thisId};
                jQuery.post(ajaxurl, data, function (response) {/* alert(response); */
                    jQuery("#ajaxContent").html(response);
                    jQuery("#ajaxstart").focus();
                });
            });
...
<form name="archivemessageform" id="archivemessageform" action="" method="post">
    <div class="wrap">
        <h2><?php _e("Message Archive", 'mailing-group-module'); ?> <?php echo($groupName != '' ? "($groupName) <a class='backlink' href='admin.php?page=wpmg_mailinggroup_list'>" . _e("Back", 'mailing-group-module') . "</a>" : "") ?></h2>
        <?php
        if (count($result) > 0) {
            ?>
            <p class="submit clear">
                <input type="submit" value="<?php _e("Delete Selected", 'mailing-group-module'); ?>" class="button"
                       id="deletemessagebtn" name="deletemessagebtn"/>
            </p>
        <?php
        }
        ?>
        <table class="wp-list-table widefat fixed" id="archivelist">
            ...
            if ($totcount > 0) {
                foreach ($result as $row) {
                    ...
                    ?>
                    <tr>
                        <td><input type="checkbox" name="deletemsg[]" id="selector" value="<?php echo $id; ?>"/></td>
                        <td><?php echo $sender; ?></td>
                        <td><?php echo $name; ?></td>
                        <td><a href="#ajaxstart" title="<?php _e("View", 'mailing-group-module'); ?>" class="quick_view"
                               name="<?php echo $emailId; ?>"><?php echo $subject; ?></a></td>
                        <td><?php echo $content; ?></td>
                        <td><?php echo wp_trim_words($content, 20, '<a href="#ajaxstart" title="Read More" class="quick_view" name="' . $emailId . '">...Read More</a>'); ?></td>
                        <td><?php echo $date; ?></td>
                        <td width="8%" class="last"><a class="delete_record"
                                                       title="<?php _e("Delete", 'mailing-group-module'); ?>"
                                                       href="admin.php?page=wpmg_mailinggroup_adminarchive&act=del&did=<?php echo $id; ?>&gid=<?php echo $gid; ?>"
                                                       onclick="return confirm('<?php _e("Are you sure you want to delete this message?", 'mailing-group-module'); ?>');"></a>|<a
                                href="#ajaxstart" class="view_record quick_view"
                                title="<?php _e("View", 'mailing-group-module'); ?>" name="<?php echo $emailId; ?>"></a>
                        </td>
                    </tr>
                <?php
                }
            ...
            <tr>
                <td colspan="6"
                    align="center"><?php _e("There are currently no messages in the archive.", 'mailing-group-module'); ?></td>
            <tr>
                <?php } ?>
            </tbody>
        </table>
