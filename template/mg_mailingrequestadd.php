...
/*TODO: CREATE USER MANAGEMENT SECTION
 get all variables */
...
<div xmlns="http://www.w3.org/1999/xhtml" class="wrap nosubsub">
	...
	<div id="col-left">
		<div class="col-wrap">
			<div>
				<p><?php _e( "Fill out the form below to add a subscriber to a mailing group.", 'mailing-group-module' ); ?>
					<br>
					<?php _e( "You will then need to activate their membership via the Subscription Request Manager.", 'mailing-group-module' ); ?>
					<br>
					<?php _e( "They will then be sent an email to confirm that they are now a subscriber.", 'mailing-group-module' ); ?>
					<br>
					<em><?php _e( "NB: Please only add subscribers here if you have their permission already.", 'mailing-group-module' ); ?></em>
				</p>
									...
									<?php foreach ( $result_groups as $group ) { ?>
										<tr>
											...
											<td>
												<div class="check_div">
													<div class="lft">
														<input type="radio" name="email_format_<?php echo $group->id; ?>" <?php echo( $email_format == '1' ? "checked" : "" ) ?> value="1" />&nbsp;<?php _e( "HTML", 'mailing-group-module' ); ?>
													</div>
													<div class="rgt">
														<input type="radio" <?php echo( $email_format == '2' ? "checked" : ( $email_format == '' ? "checked" : "" ) ) ?> name="email_format_<?php echo $group->id; ?>" value="2" />&nbsp;<?php _e( "Plain Text", 'mailing-group-module' ); ?>
													</div>
												</div>
											</td>
										</tr>
